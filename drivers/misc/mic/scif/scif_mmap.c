		   ...
		   struct scif_range **pages)
{
	...
	struct scif_window *window = NULL;
	int nr_pages, err, i;
	struct scif_range *sr = NULL;
	struct _scif_range *_sr = NULL;
	...
	/* Allocate _scif_range */
	_sr = kzalloc(sizeof(*_sr), GFP_KERNEL);
	if (!_sr) {
		err = -ENOMEM;
		goto error;
	}
	...
	sr->phys_addr = scif_zalloc(nr_pages * sizeof(dma_addr_t));
	if (!sr->phys_addr) {
		err = -ENOMEM;
		goto error;
	}
	...
	if (scif_is_mgmt_node() && !scifdev_self(ep->remote_dev)) {
		...
		sr->va = scif_zalloc(nr_pages * sizeof(void *));
		if (!sr->va) {
			err = -ENOMEM;
			goto error;
		}
	...
	_sr->cookie = window;
	_sr->sr = sr;
	sr->nr_pages = nr_pages;
	sr->prot_flags = window->prot;

	for (i = 0; i < nr_pages; i++) {
		sr->phys_addr[i] = __scif_off_to_dma_addr(window, offset +
						       (i * PAGE_SIZE));
		sr->phys_addr[i] = scif_get_phys(sr->phys_addr[i], ep);
		if (scif_is_mgmt_node() && !scifdev_self(ep->remote_dev))
			sr->va[i] = ep->remote_dev->sdev->aper->va +
				    sr->phys_addr[i] -
...
error:
	...
	if (err) {
		if (sr) {
			scif_free(sr->phys_addr, nr_pages * sizeof(dma_addr_t));
			scif_free(sr->va, nr_pages * sizeof(void *));
			kfree(sr);
		}
		kfree(_sr);
		dev_err(&ep->remote_dev->sdev->dev,
...
int scif_put_pages(struct scif_range *pages)
{
	...
	struct scif_window *window;
	struct scifmsg msg;
	struct _scif_range *_sr = container_of(&pages, struct _scif_range, sr);
