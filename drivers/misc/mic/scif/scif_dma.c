			 ...
			 bool sync_wait)
{
	...
	if (!sync_wait) {
		...
		if (DMA_COMPLETE == dma_sync_wait(chan, cookie)) {
			err = 0;
		} else {
			err = -EIO;
			dev_err(&sdev->dev, "%s %d err %d\n",
...
restart:
	...
	list_for_each_safe(item, tmp, &scif_info.rma) {
		...
		if (OP_COMPLETED == window->unreg_state) {
			if (SCIF_WINDOW_SELF == window->type)
		     ...
		     struct scif_window_iter *iter)
{
	 ...
	 */
	if (!scifdev_self(dev) && SCIF_WINDOW_PEER == window->type &&
					  ...
					  struct dma_chan *chan)
{
	...
	if (src_cache_off != 0) {
		...
		if (SCIF_WINDOW_SELF == src_window->type)
		...
		if (!src_virt)
			return -ENOMEM;
		if (SCIF_WINDOW_SELF == dst_window->type)
	...
	while (remaining_len) {
		...
		if (SCIF_WINDOW_SELF == dst_window->type)
			...
			dst_virt = ioremap_remote(dst_offset, dst_window,
						  ...
						  &dst_win_iter);
		if (!dst_virt) {
			if (SCIF_WINDOW_PEER == src_window->type)
				iounmap_remote(src_virt, loop_len, work);
			ret = -ENOMEM;
			goto error;
		}
		...
		if (SCIF_WINDOW_PEER == dst_window->type)
			iounmap_remote(dst_virt, loop_len, work);
					  ...
					  struct dma_chan *chan, off_t loffset)
{
	...
	src_local = SCIF_WINDOW_SELF == work->src_window->type;
	dst_local = SCIF_WINDOW_SELF == work->dst_window->type;
	...
	if (0 > scif_rma_list_dma_copy_unaligned(work, temp, chan, src_local))
		goto free_temp_buf;
	if (!src_local)
			 ...
			 enum scif_rma_dir dir, bool last_chunk)
{
	 ...
	 */
	remote_req.prot = SCIF_LOCAL_TO_REMOTE == dir ? VM_WRITE : VM_READ;
	...
	if (addr) {
		...
		req.prot = (SCIF_LOCAL_TO_REMOTE == dir ?
...
skip_cache:
		 ...
		 */
		req.prot = SCIF_LOCAL_TO_REMOTE == dir ? VM_READ : VM_WRITE;
		req.nr_bytes = len;
	...
	if (last_chunk) {
		...
		if (SCIF_DMA_POLL == copy_work.fence_type)
			err = scif_drain_dma_poll(rdev->sdev,
						  ep->rma_info.dma_chan);
		else if (SCIF_DMA_INTR == copy_work.fence_type)
