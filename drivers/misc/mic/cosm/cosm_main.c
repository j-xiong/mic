...
{
	mutex_lock(&cdev->cosm_mutex);
	if (MIC_READY != cdev->state || force) {
		/*
		 * Don't call hw_ops if they have been called previously.
		 * stop(..) calls device_unregister and will crash the system if
		 * called multiple times.
		 */
		bool call_hw_ops = MIC_RESET_FAILED != cdev->state &&
					MIC_READY != cdev->state;

		if (MIC_RESETTING != cdev->state)
...
static int cosm_driver_probe(struct cosm_device *cdev)
{
	...
	if (1 == atomic_add_return(1, &g_num_dev)) {
		rc = cosm_scif_init();
		if (rc)
			goto scif_exit;
	}
